...
pub struct GlobalXMajorPosition {
    // Stores global coordinates in x-major order:
    // - Top 16 bits: global_x (room_x * 50 + x)
    // - Bottom 16 bits: global_y (room_y * 50 + y)
...
impl GlobalXMajorPosition {
    ...
    pub fn new(room_x: i8, room_y: i8, x: u8, y: u8) -> Self {
        ...
        Self { 
            packed: ((global_x as u32) << 16) | (global_y as u32)
        }
    }

    #[inline]
    fn decompose_global(&self) -> (i8, i8, u8, u8) {
        let global_x = ((self.packed >> 16) & 0xFFFF) as u16;
        let global_y = (self.packed & 0xFFFF) as u16;
...
impl fast_position for GlobalXMajorPosition {
    ...
    fn xy(&self) -> RoomXY {
        ...
        let local_y = ((self.packed & 0xFFFF) % 50) as u8;
    ...
    fn y_major_global(&self) -> u32 {
        let global_x = ((self.packed >> 16) & 0xFFFF) as u16;
        let global_y = (self.packed & 0xFFFF) as u16;
        ((global_y as u32) << 16) | (global_x as u32)
    }
